...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Analyze Answer - File: ${file.name}, Type: ${file.type}, Size: ${file.size} bytes`);
    
    // Create a new form to send to the backend
    const backendForm = new FormData();
    backendForm.append('audio_file', file);
    
    // Copy all other fields from the original form
    ...
    console.error('Error in analyze-answer API route:', error);
    return Response.json(
